  ...
  methods: {
    emailNextStep() {
      this.$refs.forgetPasswordForm.validateField('email',valid => {
        if(valid){
          getRequest("/email/check?"+this.forgetPasswordForm.email,null).then((resp) => {
            if(resp.code === 2000){
              this.flag = 2;
            }else{
              this.warn = 'notExist';
    ...
    getCode() {
      ...
      getRequest('/code/get-code?'+this.forgetPasswordForm.email,null).then(resp=>{
    ...
    forgetPassword() {
      this.$refs.forgetPasswordForm.validateField('password','rePassword', valid => {
        if(valid){
          postRequest("/user/change-password",this.forgetPassword).then((resp) => {
            if(resp.code === 2000){
